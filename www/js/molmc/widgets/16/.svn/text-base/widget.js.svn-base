define(["tpl!./template/layout.tpl",
    "css!./css/style_16"], function(layoutTpl){
    var mMqtt;
    var mWidget;
    var device_id;
    var url;
...
//    };
    var getEl = function(){
        gasCanvas = $("#widget16_airCanvas")[0];
        illumination = $("#widget16_illumination");
        airTemp = $("#widget16_airTemp");
        airHumidity = $("#widget16_airHumidity");        
        canvasView();
    };
    var canvasView = function(){                
        topic = mWidget.topics;
        min_val = topic[3].attribute.min;
        max_val = topic[3].attribute.max;
        unit = topic[3].attribute.unit;
        drawBG();
        setVal(0);
    };
    ...
    var initMqtt = function(){           
        ...
        mMqtt.subscribe(url, 2, function(message){
            var str=message.payloadString.replace(/<(.+?)>/gi,"&lt;$1&gt;")
                                ...
                                .replace(/\n/gi,"<br>");
            if(airHumidity){
                airHumidity.html(parseFloat(str).toFixed(2)+" "+topic[2].attribute.unit);
            }
        });
