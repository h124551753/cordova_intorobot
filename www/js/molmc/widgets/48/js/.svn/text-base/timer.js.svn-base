...
(function($) {
    ...
    var TC_Instance = function(element, options) {
        ...
        var timerNum = 0;
        for (var key in options.time) {
            if(options.time[key].show){
                timerNum++;
            }
        }
        this.listeners = [];
        ...
        if(height === 0 && width > 0) height = width / timerNum;
        else if(width === 0 && height > 0) width = height * timerNum;
        
        this.data.attributes.canvas[0].height = height;
        this.data.attributes.canvas[0].width = width;
        this.data.attributes.canvas.appendTo(this.container);

        this.data.attributes.item_size = Math.min(this.data.attributes.canvas[0].width / timerNum, this.data.attributes.canvas[0].height);
        ...
        for (var key in this.data.text_elements) {
            if(!options.time[key].show){
                delete this.data.text_elements[key];
                delete this.data.state.fading[key];
                continue;
            }
            var textElement = $("<div>");
    ...
    TC_Instance.prototype.setOptions = function(options) {
        if(this.config === null) {
            ...
            this.config = $.extend({}, this.default_options);
        }
    ...
    TC_Class.prototype.foreach = function(callback) {
        ...
        this.elements.each(function() {
            ...
            if (typeof cur_id === "undefined" || cur_id == null) {
                cur_id = guid();
            ...
            if (typeof TC_Instance_List[cur_id] === "undefined") {
                ...
                if(typeof element_options === "object") {
                    options = $.extend({}, _this.options, element_options);
                }
    ...
    TC_Class.prototype.updata = function(num) {
        this.foreach(function(instance) {
            instance.stop();
            instance.data.attributes.timer = num;
            // instance.data.attributes.ref_date = (new Date()).getTime() + (num * 1000);
            // instance.updateArc();
            instance.start();
        });
        return this;
