define(["tpl!./template/layout.tpl", "solidgauge"], function(layoutTpl){
    ...
    var alcoholDetectChart;

    var getEl =function(){
        alcoholDetect = $("#alcoholDetect");
    };
    var initView = function(el, wid, devId, mqtt){
        ...
        el.append(div);
        getEl();
        var alcoholDetectSetting = getAttr(mWidget, 0);
        
        alcoholDetectChart = new Highcharts.Chart(Highcharts.merge(defaults, alcoholDetectSetting));
    ...
    var initMqtt = function(){
        ...
        mMqtt.on(url, function(message){            
            ...
            if(alcoholDetect){
                alcoholDetectChart.series[0].points[0].update(parseFloat(value));
            }
        });     
    };    
    ...
    var getAttr = function(options, index) {
        ...
        var renderDiv = "alcoholDetect";
        if(index===0){
            renderDiv = "alcoholDetect";
        }
    ...
    var destroy = function(){
        if(alcoholDetectChart){
            alcoholDetectChart.destroy();
        }
        var topic = mWidget.topics;
