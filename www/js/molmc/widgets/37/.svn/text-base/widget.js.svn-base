define(["tpl!./template/layout.tpl", "solidgauge"], function(layoutTpl){
    ...
    var ultravioletDetect;

    var ultravioletDetectChart;

    var getEl =function(){
        ultravioletDetect = $("#ultravioletDetect");
    };
    var initView = function(el, wid, devId, mqtt){
        ...
        el.append(div);
        getEl();
        var ultravioletDetectSetting = getAttr(mWidget, 0);
        
        ultravioletDetectChart = new Highcharts.Chart(Highcharts.merge(defaults, ultravioletDetectSetting));
    ...
    var initMqtt = function(){
        ...
        mMqtt.on(url, function(message){            
            ...
            if(ultravioletDetect){
                ultravioletDetectChart.series[0].points[0].update(parseFloat(value));
            }
        });     
    };    
    ...
    var getAttr = function(options, index) {
        ...
        var renderDiv = "ultravioletDetect";
        if(index===0){
            renderDiv = "ultravioletDetect";
        }
    ...
    var destroy = function(){
        if(ultravioletDetectChart){
            ultravioletDetectChart.destroy();
        }
        var topic = mWidget.topics;
