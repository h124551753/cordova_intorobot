define(["tpl!./template/layout.tpl", "solidgauge"], function(layoutTpl){
    ...
    var formaldehydeDetect;

    var formaldehydeDetectChart;

    var getEl =function(){
        formaldehydeDetect = $("#formaldehydeDetect");
    };
    var initView = function(el, wid, devId, mqtt){
        ...
        el.append(div);
        getEl();
        var formaldehydeDetectSetting = getAttr(mWidget, 0);
        
        formaldehydeDetectChart = new Highcharts.Chart(Highcharts.merge(defaults, formaldehydeDetectSetting));
    ...
    var initMqtt = function(){
        ...
        mMqtt.on(url, function(message){            
            ...
            if(formaldehydeDetect){
                formaldehydeDetectChart.series[0].points[0].update(parseFloat(value));
            }
        });     
    };    
    ...
    var getAttr = function(options, index) {
        ...
        var renderDiv = "formaldehydeDetect";
        if(index===0){
            renderDiv = "formaldehydeDetect";
        }
    ...
    var destroy = function(){
        if(formaldehydeDetectChart){
            formaldehydeDetectChart.destroy();
        }
        var topic = mWidget.topics;
