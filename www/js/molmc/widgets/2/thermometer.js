...
(function($){
    ...
    var img = {
        ...
        redVerticalImg: fileSystemWidget + "2/img/redVertical.png",
        tooltipFGImg: fileSystemWidget + "2/img/tickShine.png",
        glassBottomImg: fileSystemWidget + "2/img/glassBottom.png",
    ...
    var img_high = {
        ...
        redVerticalImg2x: fileSystemWidget + "2/img/redVertical2x.png",
        tooltipFGImg2x: fileSystemWidget + "2/img/tickShine2x.png",
        glassBottomImg2x: fileSystemWidget + "2/img/glassBottom2x.png",
    ...
    Thermometer.prototype = {
        thermometerCreate: function(options, el){
...
//            });
        ...
        thermometerUpdate: function(options, el) {
            animateThermometer(options, el);
        },
    };
    ...
    var createGraphics = function(el) {
        ...
        $("#widget-thermometer", el).css("height",  heightOfBody + thermTopHeight + thermBottomHeight);
        ...
        var countPerTick = (defaults.goalAmount - defaults.minAmount)/defaults.tickMarkSegementCount;
        ...
        initThermometer({currentAmount:defaults.currentAmount}, el);
    };
...
//            $("#widget-thermometer", el).fadeTo(200, 1, function(){
//                // animateThermometer({}, el);
//            });
        }
    };

    //animate the thermometer
    var initThermometer = function(options, el){
        var op = $.extend(defaults, options, true);
        var percentageComplete = (op.currentAmount - defaults.minAmount)/(defaults.goalAmount - defaults.minAmount);
    ...
    //animate the thermometer
    var animateThermometer = function(options, el){
        var op = $.extend(defaults, options, true);
        if(op.currentAmount<defaults.minAmount){
            op.currentAmount = defaults.minAmount;
        }else if(op.currentAmount>defaults.goalAmount){
            op.currentAmount=defaults.goalAmount;
        }
        var percentageComplete = (op.currentAmount - defaults.minAmount )/(defaults.goalAmount - defaults.minAmount);
    ...
    $.molmc.thermometer = new Thermometer();
})(af);